...
# we could use sine and cosine but instead we keep them as trainable weights with a bit of dropout.
# Its weights will be learned as we train the model, all to decrease the loss function.
...
# sive of vector embeddings, i.e. dimensions 512. 500 is the max token sequence length. 256 is the batch size
embed_dim = 512
...
# multihead attention!!!
# so, the heart of transformer.
# We're using scaled dot product attention as we divide by d**0.5

class MultiheadAttention(nn.Module):
    # defining the query, key, value architecture

    def __init__(self, embed_dim, num_heads, dropout=0.1):
        super().__init__()
        self.h = num_heads # keeping track of head number
        self.d = embed_dim // num_heads # calculating number of dimensions per head, we'll use 4 heads so it will be nicely divisible
        # think of this as we split 512 dimensions into 4 attention layers, 128 each,
        # so each head could possibly specialize in different features

        # just defining four linear models 512 dims in and out.
    ...
    def split_heads(self, X):
        # reshaping data
        # first [256, L, 512]  →  [256, L, 4, 128]
        # then  [256, L, 4, 128]  →  [256, 4, L, 128]
        return X.view(X.size(0), X.size(1), self.h, self.d).transpose(1, 2)

    # defining the forward pass; for representing shapes values are:
    # B - batch size, h - number of heads, Lq, Lk, Lv -- max query, key, value length in batch, d - embed_dim/num heads
    # B=256; h=4; d=512/4=128;

    def forward(self, query, key, value, attn_mask=None, key_padding_mask=None):
        # each QKV goes first through linear learned model _proj, then through data reshaping
...
# putting encoder decoder layers into modules
...
# lastly the transformer module itself:
...
torch.save(nmt_tr_model.state_dict(), "transformer_nmt_model.pt")
...
nmt_tr_model.eval()
translate(nmt_tr_model, "Good morning!")

nmt_tr_model.eval()
translate(nmt_tr_model, "How are you?")

nmt_tr_model.eval()
translate(nmt_tr_model, "South park is the greatest series in the world, it entirely transformed how we view the role of mass media in society")

nmt_tr_model.eval()
translate(nmt_tr_model, "Technical AI safety is an important growing field in AI, plausible the most important thing to be working on in the whole world!")
